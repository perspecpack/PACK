-- Adiciona colunas de auditoria/metadados de histórico temporário
alter table public.process_validation_responses add column if not exists pdf_hash text;
alter table public.process_validation_responses add column if not exists materials_hashes jsonb;
alter table public.process_validation_responses add column if not exists client_attachments_count integer default 0;
alter table public.process_validation_responses add column if not exists client_attachments_names text[];
alter table public.process_validation_responses add column if not exists validation_source text default 'web';
alter table public.process_validation_responses add column if not exists metadata jsonb default '{}'::jsonb;

-- Recria a RPC submit_validation_response incluindo suporte às novas colunas e evitando conflitos com campos preenchidos pela empresa
...
begin
    ...
    loop
        ...
        -- Ignore informational text blocks and blocks filled by company
